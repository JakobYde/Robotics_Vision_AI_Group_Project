...
void Lidar::setRadius(double val) {
    maxRadius = MAX(val, maxRadius);
}

void Lidar::drawProcess() {
    cv::Mat img(cv::Size(1000, 1000), CV_8UC3);

    for (PolarPoint pt : points) cv::circle(img, ((pt.asPoint() + Point(maxRadius,maxRadius)) * 500.0 / maxRadius).getCVPoint(), 3, cv::Vec3b(0,255,0) );

    std::vector<Line> lines = getLines();

    for (Line l : lines) {
        Edge e = l.asEdge(Point(-maxRadius, -maxRadius), Point(maxRadius, maxRadius));
        cv::line(img, e.A.getCVPoint(), e.B.getCVPoint(), cv::Vec3b(0,0,255), 3);
    }
